...
export default function ProjectsView({ projects }: { projects: ProjectCardData[] }) {
  const [updatesByProject, setUpdatesByProject] = useState<Record<number, Update[]>>(() => {
  ...
  const [messageByProject, setMessageByProject] = useState<Record<number, string>>({});
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {cards.map((p) => {
          ...
          const team = Array.from(new Set(updates.map((u) => u.author))).slice(0, 3);
            ...
            <div key={p.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="text-xs text-zinc-400 mt-1">Client: {p.client_name}</div>
                </div>
                <a
              ...
              <div className="mt-4 flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                  {team.length ? (
                    team.map((name) => <Avatar key={name} name={name} size={26} />)
                  ) : (
                    <div className="text-xs text-zinc-500">No assignees yet</div>
                  )}
                </div>
              </div>
              ...
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-3 sm:col-span-1">
                  ...
                  <div className="text-xs text-zinc-500 mt-1">
                    {p.github?.lastCommit?.date ? formatDate(p.github.lastCommit.date) : ""}
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div className="sm:col-span-1">
                  <label className="block text-xs text-zinc-400 mb-1">New update</label>
                  <input
                    value={messageByProject[p.id] ?? ""}
                    onChange={(e) => setMessageByProject((prev) => ({ ...prev, [p.id]: e.target.value }))}
                    className="w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm outline-none focus:border-zinc-600"
